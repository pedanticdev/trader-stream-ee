...
public class GCStatsService implements NotificationListener {
   ...
   *
   * <p>Not all JVMs expose detailed phase information via GcInfo. When unavailable, the breakdown
   * will contain only a "Total" phase with the full pause duration. This is a best-effort
   * extraction that varies by GC implementation (G1, C4, ZGC, etc.).
   */
  public static class GCPhaseBreakdown {
    ...
    public GCPhaseBreakdown(
        long totalDurationMs, Map<String, Long> phaseDurationsMs, long timestamp) {
   ...
   *
   * <p>Provides percentile distribution of phase durations to identify which phases dominate pause
   * times.
   */
  public static class PhaseStats {
    ...
    public PhaseStats(
        String phaseName, long count, long p50Ms, long p95Ms, long p99Ms, long maxMs) {
  ...
  public void handleNotification(Notification notification, Object handback) {
    if (notification
      ...
      // Phase breakdown extraction (new logic)
      Map<String, Long> phaseTimes = extractPhaseTimes(gcInfo, gcName, gcAction);
      GCPhaseBreakdown breakdown =
          new GCPhaseBreakdown(duration, phaseTimes, System.currentTimeMillis());

      ConcurrentLinkedDeque<GCPhaseBreakdown> phaseHist =
          phaseHistory.computeIfAbsent(gcName, k -> new ConcurrentLinkedDeque<>());
   ...
   *
   * <p>This is JVM and collector-specific - detailed phase data may not be available on all
   * platforms. The standard GcInfo API does not expose phase-level timing; vendors may provide this
   * via custom MBeans or extended attributes.
   *
   * <p>For production-grade phase analysis, consider parsing GC logs with -Xlog:gc*=debug or using
   * vendor-specific monitoring tools.
   *
   * @param gcInfo GC information from notification
   * @param gcName Name of garbage collector (e.g., "G1 Young Generation", "GPGC Pauses")
   * @param gcAction GC action string (e.g., "end of minor GC", "end of major GC")
   * @return Map of phase names to durations in milliseconds. Contains at minimum a "Total" entry
   *     with the full pause duration.
   */
   ...
   *
   * <p>Groups phase durations by phase name and computes percentiles for each phase independently.
   * This allows identifying which phases contribute most to pause times (e.g., "Mark" vs "Evacuate"
   * in G1GC).
   *
   * @param breakdowns List of GC phase breakdowns from notification history
    ...
    for (Map.Entry<String, List<Long>> entry : phaseGroups.entrySet()) {
      ...
      PhaseStats stats =
          new PhaseStats(
              ...
              durations.get(durations.size() - 1) // max
              );
