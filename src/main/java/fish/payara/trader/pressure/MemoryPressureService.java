...
/**
 * Service to generate controlled memory pressure for GC stress testing. Updated to use scenario-based testing targeting G1 vs C4 differences.
 */
@ApplicationScoped
public class MemoryPressureService {
    ...
    /**
     * Holder object that lives in old generation and holds a reference to a young object. Every update to youngRef triggers G1's write barrier and remembered
     * set maintenance.
     */
    ...
    /**
     * Maintain RefHolder objects that will live in old generation. Each holder has ~1MB padding and a reference slot for young objects.
     */
    ...
    /**
     * Create young objects and update old→young references. Each reference update triggers G1's write barrier, which marks the card table and adds to
     * remembered sets. This is the overhead we're testing - C4 has no remembered sets.
     */
    ...
    private void logStats() {
        ...
        if (now - lastStatsTime >= 5000) {
            ...
            LOGGER.info(String.format("Memory Pressure Stats - Mode: %s | Allocated: %.2f MB/sec | Live Set: %d MB", currentMode, mbPerSec,
                            liveSetBytesAllocated.get() / (1024 * 1024)));
