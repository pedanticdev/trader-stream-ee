...
public class StatusResource {
    ...
    public Response getClusterStatus() {
        ...
        try {
            ...
            clusterInfo.put("members",
                            hazelcastInstance.getCluster()
                                            .getMembers()
                                            .stream()
                                            .map(member -> Map.of("address", member.getAddress().toString(), "uuid", member.getUuid().toString(), "localMember",
                                                            member.localMember(), "liteMember", member.isLiteMember()))
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(clusterInfo).build();
        }
    }
}
