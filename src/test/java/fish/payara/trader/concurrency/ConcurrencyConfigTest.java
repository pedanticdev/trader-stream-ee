...
class ConcurrencyConfigTest {
    ...
    void shouldHaveCorrectExecutorDefinitionProperties() {
        ...
        // Check executor name
        assertEquals("java:module/concurrent/VirtualThreadExecutor", annotation.name(),
            "Executor name should match expected value");
        ...
        assertEquals(VirtualThreadExecutor.class, annotation.qualifiers()[0],
            "Should use VirtualThreadExecutor qualifier");
    }
    ...
    void shouldBeAConcreteClass() {
        // Assert
        assertFalse(ConcurrencyConfig.class.isInterface(),
            "ConcurrencyConfig should be a concrete class");
        assertFalse(java.lang.reflect.Modifier.isAbstract(ConcurrencyConfig.class.getModifiers()),
            "ConcurrencyConfig should not be abstract");
        assertTrue(ConcurrencyConfig.class.isSynthetic() == false,
            "ConcurrencyConfig should not be synthetic");
    }
    ...
    void shouldBeInCorrectPackage() {
        // Assert
        assertEquals("fish.payara.trader.concurrency", ConcurrencyConfig.class.getPackage().getName(),
    ...
    void shouldHaveCorrectSimpleName() {
        // Assert
        assertEquals("ConcurrencyConfig", ConcurrencyConfig.class.getSimpleName(),
            "Should have correct simple name");
    }
    ...
    void shouldBeAnnotatedWithProperLifecycleScope() {
        ...
        assertEquals("jakarta.enterprise.context.ApplicationScoped", applicationScoped.annotationType().getName(),
            "Should use Jakarta EE ApplicationScoped");
    }
    ...
    void shouldConfigureVirtualThreadsForExecutor() {
        ...
        assertEquals("java:module/concurrent/VirtualThreadExecutor", annotation.name(),
            "Should use specific jndi name for virtual thread executor");
    }
    ...
    void shouldHaveVirtualThreadExecutorQualifier() {
        ...
        assertEquals(1, annotation.qualifiers().length, "Should have exactly one qualifier");
        assertEquals(VirtualThreadExecutor.class, annotation.qualifiers()[0],
            "Should use VirtualThreadExecutor as qualifier");
    }
    ...
    void shouldHaveCorrectJNDINamePattern() {
        ...
        assertEquals("java:module/concurrent/VirtualThreadExecutor", jndiName,
            "JNDI name should follow expected pattern");
        assertTrue(jndiName.startsWith("java:module/concurrent/"),
            "JNDI name should start with expected prefix");
        assertTrue(jndiName.endsWith("VirtualThreadExecutor"),
            "JNDI name should end with expected suffix");
    ...
    void shouldHaveNoDeclaredMethods() {
        ...
        long userMethods = java.util.Arrays.stream(methods)
    ...
    void shouldBeProperlyConfiguredForVirtualThreads() {
        ...
        assertEquals("java:module/concurrent/VirtualThreadExecutor", definition.name(),
            "Should have correct JNDI name for virtual threads");

        // Verify it uses VirtualThreadExecutor qualifier
        assertEquals(1, definition.qualifiers().length);
        assertEquals(VirtualThreadExecutor.class, definition.qualifiers()[0],
            "Should use VirtualThreadExecutor qualifier");
    }
}