...
class ConcurrencyConfigTest {
  ...
  void shouldHaveManagedExecutorDefinitionAnnotation() {
    // Act
    ManagedExecutorDefinition annotation =
        ConcurrencyConfig.class.getAnnotation(ManagedExecutorDefinition.class);
  ...
  void shouldHaveCorrectExecutorDefinitionProperties() {
    ...
    // Check executor name
    assertEquals(
        "java:module/concurrent/VirtualThreadExecutor",
        annotation.name(),
  ...
  void shouldBeAConcreteClass() {
    // Assert
    assertFalse(
        ConcurrencyConfig.class.isInterface(), "ConcurrencyConfig should be a concrete class");
    assertFalse(
        java.lang.reflect.Modifier.isAbstract(ConcurrencyConfig.class.getModifiers()),
        "ConcurrencyConfig should not be abstract");
    assertTrue(
        ConcurrencyConfig.class.isSynthetic() == false,
  ...
  void shouldBeInCorrectPackage() {
    ...
    assertEquals(
        ...
        ConcurrencyConfig.class.getPackage().getName(),
  ...
  void shouldHaveCorrectSimpleName() {
    ...
    assertEquals(
        "ConcurrencyConfig",
        ConcurrencyConfig.class.getSimpleName(),
  ...
  void shouldBeAnnotatedWithProperLifecycleScope() {
    ...
    ApplicationScoped applicationScoped =
        ConcurrencyConfig.class.getAnnotation(ApplicationScoped.class);

    // Assert
    assertNotNull(applicationScoped, "Should have ApplicationScoped annotation");
    assertEquals(
        "jakarta.enterprise.context.ApplicationScoped",
        applicationScoped.annotationType().getName(),
  ...
  void shouldHaveCorrectJNDINamePattern() {
    ...
    assertTrue(
        jndiName.startsWith("java:module/concurrent/"),
        "JNDI name should start with expected prefix");
    assertTrue(
        jndiName.endsWith("VirtualThreadExecutor"), "JNDI name should end with expected suffix");
  ...
  void shouldHaveNoDeclaredMethods() {
    ...
    long userMethods =
        java.util.Arrays.stream(methods).filter(method -> !method.isSynthetic()).count();
  ...
  void shouldBeSuitableForCDIBeanRegistration() {
    ...
    ManagedExecutorDefinition managedExecutorDefinition =
        ConcurrencyConfig.class.getAnnotation(ManagedExecutorDefinition.class);
  ...
  void shouldBeProperlyConfiguredForVirtualThreads() {
    // Act
    ManagedExecutorDefinition definition =
        ConcurrencyConfig.class.getAnnotation(ManagedExecutorDefinition.class);
