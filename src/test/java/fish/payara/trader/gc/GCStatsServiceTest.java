...
class GCStatsServiceTest {
    ...
    void shouldHandleGCNotificationAndRecordPause() {
        Notification notification = new Notification(GarbageCollectionNotificationInfo.GARBAGE_COLLECTION_NOTIFICATION, "test", 1L);
        ...
        try (MockedStatic<GarbageCollectionNotificationInfo> mockedStatic = mockStatic(GarbageCollectionNotificationInfo.class)) {
            mockedStatic.when(() -> GarbageCollectionNotificationInfo.from(compositeData)).thenReturn(info);
        ...
        GCStats g1Stats = stats.stream().filter(s -> s.getGcName().equals("G1 Young Generation")).findFirst().orElse(null);
    ...
    void shouldFilterGPGCConcurrentCycleNotifications() {
        ...
        List<GCStats> stats = gcStatsService.collectGCStats();
        GCStats gpgcStats = stats.stream().filter(s -> s.getGcName().equals("GPGC")).findFirst().orElse(null);
