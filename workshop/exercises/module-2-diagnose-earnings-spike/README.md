# Module 2 exercise: diagnose EARNINGS_SPIKE

## Problem

The `EARNINGS_SPIKE` scenario simulates a post-earnings directional flow: 300 MB/sec allocation with roughly 50% order survival into old generation. Use JFR to explain *why* this produces increasing pause times on the G1 runtime, and *why* the ZGC runtime does not respond the same way.
...
3. **`Event Browser → jdk.PromoteObjectOutsidePLAB`**: events fire when an object is promoted to old gen but the PLAB (promotion buffer) did not have room. High frequency means the application is over-allocating into PLABs, which is the post-earnings directional-flow signature on G1.
...
Repeat for the ZGC recording. The same heap behaviour is present (orders are being promoted), but pause time should not move.

## Fill this in

```
G1 EARNINGS_SPIKE observations:
...
ZGC EARNINGS_SPIKE observations:
...
G1's old-gen collection is stop-the-world (the mixed phases). Once old gen fills, G1 must run mixed collections, and their cost scales with the live data they have to evacuate. The earnings spike builds the old gen up faster than concurrent marking can keep up, so mixed collections trigger and produce pauses.

ZGC's collection runs concurrently with the application. Promotion into old gen does not cause a stop-the-world phase. Heap fills the same way; the application does not pause.
